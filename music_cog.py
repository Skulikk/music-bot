...
class music(commands.Cog):
    ...
    async def shuffle_queue(self):
        ...
        predownloaded_loc = getattr(self, "predownloaded")
        print(predownloaded_loc)
    ...
    def YT_search(self, query, direct):
        ...
        if re.match(r"^(?:https?:)?(?:\/\/)?(?:youtu\.be\/|(?:www\.|m\.)?youtube\.com\/(?:watch|v|embed)(?:\.php)?(?:\?.*v=|\/))([a-zA-Z0-9\_-]{7,15})(?:[\?&][a-zA-Z0-9\_-]+=[a-zA-Z0-9\_-]+)*(?:[&\/\#].*)?$", query):
            ...
            #no result
            if (not res):
                #await self.error_message("Skladba " + query + " nebyla nalezena")
                return 0
    ...
    async def send_queue_embed(self):
        ...
        try:
            ...
            pass
        self.embed_queue = discord.Embed(title="Fronta", color=0x0000ff)

        if (len(self.song_queue) > 24):
            if (len(self.queue_for_queue) > 0):
                self.embed_queue.set_footer(text="Skladeb ve frontě: " + str(len(self.song_queue)) + "\nFronta zobrazuje pouze prvních 24 skladeb.\nProbíhá přidávání skladeb do fronty. Zbývá přidat: " + str(len(self.queue_for_queue)))
            else:    
                self.embed_queue.set_footer(text="Skladeb ve frontě: " + str(len(self.song_queue)) + "\nFronta zobrazuje pouze prvních 24 skladeb.")
        else:
            if (len(self.queue_for_queue) > 0):
        ...
        if (len(self.song_queue)) >= 1:
            ...
            self.embed_queue.add_field(name="Fronta je prázdná, další skladbu přidej příkazem !play ...", value="", inline=False)

        #queue msg already exists
        if (self.queue_message):
            await self.queue_message.edit(embed=self.embed_queue)
        else:
            button_view=View_queue_buttons(self.bot, self.ctx.guild, self.song_queue, self.ctx, None, self.predownloaded, timeout=None)
    ...
    async def play_task(self, voice, duration, message, name, thumbnail):
        ...
        while(not self.task_done):
            if(self.predownloaded and not predownload_sent):
                embed_player.set_footer(text="Next: " + self.song_queue[0][1] + " (preloaded)")
                await message.edit(embed=embed_player)
                predownload_sent = 1

            print(self.predownloaded)
            print(predownload_sent)

            if(not self.predownloaded and predownload_sent):
                embed_player.remove_footer()
                embed_player.set_footer(text="Next: " + self.song_queue[0][1])
                await message.edit(embed=embed_player)
                predownload_sent = 1
                embed_player.remove_footer
    ...
    async def play(self, ctx:commands.Context, *args):
        ...
        if args[0] == "-q":
            if len(args) == 2:
                ...
                if (int(args[1]) > 20 or int(args[1]) < 2):
                    await self.error_message("***Špatná hodnota přepínače -q.***")
                    return
            else:
                length = 10
            ...
            #    #await self.error_message("***V databázi není dostatek záznamů.***")
            #    return
        ...
        if not(voice and voice.is_connected()):
            ...
            await self.send_queue_embed()
            message = await ctx.send(embed=embed_player, view=View_buttons(self.bot.voice_clients, ctx.guild, self.song_queue, ctx, self.queue_message, timeout=None))
...
class View_buttons(discord.ui.View, music):
    def __init__(self, voice_clients, voice_guilt, song_queue, ctx, queue_message, *args, **kwargs):
    ...
    async def next_callback(self, interaction: discord.Interaction, button: discord.ui.button):
        if (not self.song_queue):
           await interaction.response.send_message(content="Fronta je prázdná", ephemeral=True, delete_after = 5)
           return
        self.voice.stop()
        await interaction.response.defer()
...
class View_queue_buttons(discord.ui.View, music):
    def __init__(self, bot, voice_guilt, song_queue, ctx, queue_message, predownloaded, *args, **kwargs):
